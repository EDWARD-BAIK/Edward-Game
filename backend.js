...
let gameSettings = Object.assign({
    ...
    particlesOn: true, screenShakeOn: true
...
function triggerShake(mag) { if (gameSettings.screenShakeOn) screenShakeMag = Math.max(screenShakeMag, mag); }
...
function startMusic() {
    ...
    try {
        ...
        master.gain.setTargetAtTime(gameSettings.musicVolume * 0.28, audioCtx.currentTime, 2);
        ...
        function playChord() {
            ...
            notes.forEach((freq, i) => {
                ...
                osc.start();
                g.gain.setTargetAtTime(0.16 / notes.length, audioCtx.currentTime, 1.8);
...
function applyMusicVolume() {
    if (musicNodes && audioCtx) musicNodes.master.gain.setTargetAtTime(gameSettings.musicOn ? gameSettings.musicVolume * 0.28 : 0, audioCtx.currentTime, 0.3);
}
...
document.getElementById('inGameSettingsBtn').addEventListener('click', openSettings);
