...
document.getElementById('openLeaderboardBtn').addEventListener('click', () => {
    ...
    document.getElementById('onlineLeaderboardHint').style.display = 'none';
...
document.getElementById('tabOnlineBtn').addEventListener('click', () => {
    ...
    document.getElementById('onlineLeaderboardHint').style.display = 'block';
});
...
function openSettings(inGame = false) {
    ...
    document.getElementById('resetProgressBtn').style.display = inGame ? 'none' : 'block';
    document.getElementById('settingsModal').style.display = 'flex';
    if (inGame) {
        pauseGame();
        document.getElementById('settingsTitle').innerText = isMultiplayer ? "⚙ PENGATURAN" : "GAME DIJEDA";
    } else {
...
document.getElementById('resetProgressBtn').addEventListener('click', () => {
    ...
    showConfirm('KAMU YAKIN INGIN RESET PROGRES', 'Koin, skin, rekor jarak, misi, dan pencapaian akan dihapus permanen.', () => {
        ['edward_total_coins','edward_owned_skins','edward_selected_skin','edward_high_dist','edward_leaderboard','edward_online_players','edward_stats','edward_achievements','edward_daily','edward_peer_id','edward_dimension_mode'].forEach(k => localStorage.removeItem(k));
...
function initPeer() {
    if (typeof Peer === 'undefined') {
        console.error('PeerJS gagal dimuat (cek koneksi internet). Mode multiplayer dinonaktifkan.');
        let idEl = document.getElementById('myIdDisplay');
        if (idEl) idEl.innerText = "Tidak tersedia (offline)";
        return;
    }
...
function handleMpData(data) {
    ...
    switch (data.type) {
        ...
        case 'start':
            mpMode = data.mode;
            mpSeed = data.seed;
            mpDimensionMode = data.dimensionMode || 'random';
...
function updateLobbyUI() {
    ...
    if (myReady && oppReady && isHost && mpConn) {
        ...
        mpMode = gameMode;
        mpSeed = seed;
        mpDimensionMode = dimensionMode;
        mpConn.send({ type: 'start', seed: seed, mode: mpMode, dimensionMode: dimensionMode });
        beginMultiplayerMatch();
    }
}
...
function pauseGame() {
    ...
    if (isMultiplayer) { showToast("Tidak bisa jeda saat mode multiplayer"); return; }
    isPaused = true;
    cancelAnimationFrame(animId);
...
let dpr = 1, viewportW = gameContainer.clientWidth, viewportH = gameContainer.clientHeight;

function resizeCanvas() {
    viewportW = gameContainer.clientWidth; viewportH = gameContainer.clientHeight;
...
const themes = [
    { type: 'earth', name: "Bumi", plat: "#1e293b", top: "#00d2d3", lava: "#ff4500", sky1: "#38bdf8", sky2: "#bae6fd",
      mountain: "rgba(15, 23, 42, 0.4)", ambientColor: "255,255,255", ambientMode: 'rise', ambientShape: 'circle', bgStyle: 'mountain' },
    { type: 'cyber', name: "Kota Neon", plat: "#2f002c", top: "#ff007f", lava: "#00ffff", sky1: "#240046", sky2: "#10002b",
      mountain: "rgba(20, 0, 40, 0.55)", ambientColor: "0,255,255", ambientMode: 'glitch', ambientShape: 'square', bgStyle: 'city' },
    { type: 'alien', name: "Planet Asing", plat: "#1a3622", top: "#00e676", lava: "#c51162", sky1: "#3b0764", sky2: "#180325",
      mountain: "rgba(10, 30, 15, 0.5)", ambientColor: "173,255,120", ambientMode: 'rise', ambientShape: 'circle', bgStyle: 'spikes' },
    { type: 'ice', name: "Kutub Beku", plat: "#1e3a4a", top: "#e0f7fa", lava: "#4fc3f7", sky1: "#7fd8f7", sky2: "#e8fbff",
      mountain: "rgba(200, 240, 255, 0.35)", ambientColor: "255,255,255", ambientMode: 'fall', ambientShape: 'circle', bgStyle: 'iceberg' },
    { type: 'volcano', name: "Gunung Magma", plat: "#2b1a12", top: "#ff5722", lava: "#ff9100", sky1: "#3d0d0d", sky2: "#1a0505",
      mountain: "rgba(40, 5, 5, 0.55)", ambientColor: "255,140,60", ambientMode: 'rise', ambientShape: 'circle', bgStyle: 'volcano' },
    { type: 'ocean', name: "Lautan Dalam", plat: "#062f4a", top: "#26c6da", lava: "#014f86", sky1: "#012a4a", sky2: "#01497c",
      mountain: "rgba(1, 30, 50, 0.5)", ambientColor: "150,230,255", ambientMode: 'rise', ambientShape: 'circle', bgStyle: 'coral' },
    { type: 'desert', name: "Gurun Pasir", plat: "#7a5230", top: "#ffca7a", lava: "#c1440e", sky1: "#ffb347", sky2: "#ffe1a8",
      mountain: "rgba(120, 70, 30, 0.4)", ambientColor: "255,224,150", ambientMode: 'fall', ambientShape: 'circle', bgStyle: 'dunes' },
    { type: 'galaxy', name: "Angkasa Luar", plat: "#12082e", top: "#b388ff", lava: "#651fff", sky1: "#050014", sky2: "#1a0940",
      mountain: "rgba(10, 5, 30, 0.6)", ambientColor: "200,180,255", ambientMode: 'drift', ambientShape: 'circle', bgStyle: 'planets' },
    { type: 'candy', name: "Dunia Permen", plat: "#ff8fab", top: "#ffe1f0", lava: "#ff5c8a", sky1: "#ffd6e8", sky2: "#fff0f7",
      mountain: "rgba(255, 180, 210, 0.4)", ambientColor: "255,255,255", ambientMode: 'fall', ambientShape: 'circle', bgStyle: 'mushroom' },
    { type: 'glitch', name: "Dimensi Glitch", plat: "#0a0a0a", top: "#39ff14", lava: "#ff003c", sky1: "#000000", sky2: "#1a001a",
      mountain: "rgba(20, 0, 20, 0.6)", ambientColor: "57,255,20", ambientMode: 'glitch', ambientShape: 'square', bgStyle: 'glitchblocks' },
    { type: 'crystal', name: "Kristal Ajaib", plat: "#2d1b4e", top: "#a78bfa", lava: "#7c3aed", sky1: "#4c1d95", sky2: "#ede9fe",
      mountain: "rgba(76, 29, 149, 0.4)", ambientColor: "200,180,255", ambientMode: 'drift', ambientShape: 'circle', bgStyle: 'crystal' },
    { type: 'toxic', name: "Rawa Beracun", plat: "#1a2e1a", top: "#84cc16", lava: "#4d7c0f", sky1: "#1a2e05", sky2: "#3f6212",
      mountain: "rgba(20, 40, 10, 0.5)", ambientColor: "190,255,80", ambientMode: 'rise', ambientShape: 'circle', bgStyle: 'spikes' },
    { type: 'sakura', name: "Sakura Musim Semi", plat: "#4a2c3a", top: "#ffb7c5", lava: "#ff69b4", sky1: "#ffe0ec", sky2: "#fff5f8",
      mountain: "rgba(255, 150, 180, 0.3)", ambientColor: "255,200,220", ambientMode: 'fall', ambientShape: 'circle', bgStyle: 'trees' },
    { type: 'storm', name: "Badai Petir", plat: "#1f2937", top: "#fbbf24", lava: "#eab308", sky1: "#111827", sky2: "#374151",
      mountain: "rgba(30, 30, 40, 0.6)", ambientColor: "250,240,150", ambientMode: 'glitch', ambientShape: 'square', bgStyle: 'towers' },
    { type: 'underworld', name: "Dunia Bawah", plat: "#1a0a0a", top: "#dc2626", lava: "#7f1d1d", sky1: "#000000", sky2: "#2d0a0a",
      mountain: "rgba(40, 0, 0, 0.6)", ambientColor: "255,80,80", ambientMode: 'rise', ambientShape: 'circle', bgStyle: 'spikes' },
    { type: 'coral', name: "Terumbu Karang", plat: "#023047", top: "#ff6b6b", lava: "#ee6c4d", sky1: "#006494", sky2: "#90e0ef",
      mountain: "rgba(0, 60, 80, 0.4)", ambientColor: "255,180,150", ambientMode: 'drift', ambientShape: 'circle', bgStyle: 'coral' },
    { type: 'aurora', name: "Cahaya Aurora", plat: "#0d1b2a", top: "#56cfe1", lava: "#64dfdf", sky1: "#03071e", sky2: "#22223b",
      mountain: "rgba(10, 20, 40, 0.55)", ambientColor: "150,255,220", ambientMode: 'drift', ambientShape: 'circle', bgStyle: 'iceberg' },
    { type: 'ruins', name: "Reruntuhan Kuno", plat: "#6b4423", top: "#d4a373", lava: "#9c6644", sky1: "#edc9a0", sky2: "#faedcd",
      mountain: "rgba(90, 60, 30, 0.4)", ambientColor: "230,200,150", ambientMode: 'fall', ambientShape: 'circle', bgStyle: 'ruins' },
    { type: 'mirror', name: "Dimensi Cermin", plat: "#e5e5e5", top: "#ffffff", lava: "#a1a1aa", sky1: "#f4f4f5", sky2: "#ffffff",
      mountain: "rgba(150, 150, 150, 0.3)", ambientColor: "200,200,255", ambientMode: 'glitch', ambientShape: 'square', bgStyle: 'crystal' },
    { type: 'jungle', name: "Hutan Terlarang", plat: "#14281d", top: "#2d6a4f", lava: "#40916c", sky1: "#081c15", sky2: "#1b4332",
      mountain: "rgba(10, 30, 20, 0.5)", ambientColor: "150,255,180", ambientMode: 'rise', ambientShape: 'circle', bgStyle: 'trees' },
    { type: 'autumn', name: "Musim Gugur Emas", plat: "#5c3a21", top: "#d97706", lava: "#b45309", sky1: "#fb923c", sky2: "#fde68a",
      mountain: "rgba(120, 60, 20, 0.4)", ambientColor: "251,146,60", ambientMode: 'fall', ambientShape: 'circle', bgStyle: 'trees' },
    { type: 'gold', name: "Kerajaan Emas", plat: "#78350f", top: "#facc15", lava: "#eab308", sky1: "#fef08a", sky2: "#fde047",
      mountain: "rgba(120, 90, 10, 0.4)", ambientColor: "255,223,100", ambientMode: 'drift', ambientShape: 'circle', bgStyle: 'towers' },
    { type: 'nebula', name: "Nebula Ungu", plat: "#1e0a3c", top: "#c084fc", lava: "#9333ea", sky1: "#0f0326", sky2: "#3b0764",
      mountain: "rgba(40, 10, 70, 0.55)", ambientColor: "216,180,254", ambientMode: 'drift', ambientShape: 'circle', bgStyle: 'planets' },
    { type: 'atlantis', name: "Reruntuhan Atlantis", plat: "#083344", top: "#22d3ee", lava: "#0e7490", sky1: "#012a3a", sky2: "#075985",
      mountain: "rgba(5, 40, 60, 0.5)", ambientColor: "125,211,252", ambientMode: 'rise', ambientShape: 'circle', bgStyle: 'ruins' },
    { type: 'bioluminescent', name: "Rimba Bercahaya", plat: "#052e16", top: "#4ade80", lava: "#15803d", sky1: "#01130a", sky2: "#022c14",
      mountain: "rgba(5, 40, 20, 0.55)", ambientColor: "134,239,172", ambientMode: 'drift', ambientShape: 'circle', bgStyle: 'trees' },
    { type: 'timerift', name: "Celah Waktu", plat: "#111827", top: "#facc15", lava: "#eab308", sky1: "#0a0a0a", sky2: "#27272a",
      mountain: "rgba(20, 20, 20, 0.6)", ambientColor: "250,204,21", ambientMode: 'glitch', ambientShape: 'square', bgStyle: 'glitchblocks' },
    { type: 'nightforest', name: "Hutan Kristal Malam", plat: "#0f172a", top: "#818cf8", lava: "#4f46e5", sky1: "#020617", sky2: "#1e1b4b",
      mountain: "rgba(10, 10, 40, 0.55)", ambientColor: "165,180,252", ambientMode: 'drift', ambientShape: 'circle', bgStyle: 'crystal' },
    { type: 'mars', name: "Gurun Merah Mars", plat: "#7c2d12", top: "#ea580c", lava: "#9a3412", sky1: "#450a0a", sky2: "#7c2d12",
      mountain: "rgba(90, 20, 10, 0.5)", ambientColor: "251,146,60", ambientMode: 'fall', ambientShape: 'circle', bgStyle: 'dunes' },
    { type: 'wasteland', name: "Kota Terbengkalai", plat: "#292524", top: "#78716c", lava: "#57534e", sky1: "#1c1917", sky2: "#44403c",
      mountain: "rgba(30, 30, 25, 0.6)", ambientColor: "168,162,158", ambientMode: 'fall', ambientShape: 'square', bgStyle: 'city' },
    { type: 'heaven', name: "Surga Awan", plat: "#e0f2fe", top: "#ffffff", lava: "#bae6fd", sky1: "#dbeafe", sky2: "#fff7ed",
      mountain: "rgba(255, 255, 255, 0.4)", ambientColor: "255,255,255", ambientMode: 'drift', ambientShape: 'circle', bgStyle: 'towers' },
    { type: 'savanna', name: "Savana Senja", plat: "#8b5a2b", top: "#f4a825", lava: "#d2691e", sky1: "#ff8c42", sky2: "#ffd97d",
      mountain: "rgba(120, 70, 20, 0.4)", ambientColor: "255,200,120", ambientMode: 'drift', ambientShape: 'circle', bgStyle: 'trees' },
    { type: 'temple', name: "Kuil Batu Purba", plat: "#4a4238", top: "#c9a876", lava: "#8b6914", sky1: "#6b5b45", sky2: "#c9b896",
      mountain: "rgba(60, 50, 40, 0.5)", ambientColor: "230,210,170", ambientMode: 'fall', ambientShape: 'circle', bgStyle: 'ruins' },
    { type: 'sandstorm', name: "Badai Pasir", plat: "#6b4f2a", top: "#d4a45a", lava: "#a0733a", sky1: "#c9974a", sky2: "#e8c98a",
      mountain: "rgba(100, 75, 40, 0.6)", ambientColor: "220,190,140", ambientMode: 'glitch', ambientShape: 'square', bgStyle: 'dunes' },
    { type: 'mistyforest', name: "Hutan Kabut", plat: "#2f3e34", top: "#8fae9b", lava: "#4a6558", sky1: "#a8bdb2", sky2: "#dfe8e2",
      mountain: "rgba(50, 70, 60, 0.5)", ambientColor: "200,220,210", ambientMode: 'drift', ambientShape: 'circle', bgStyle: 'trees' },
    { type: 'undersea', name: "Kota Bawah Laut", plat: "#032f42", top: "#38b6d4", lava: "#0a5c78", sky1: "#013a52", sky2: "#0a6485",
      mountain: "rgba(5, 50, 70, 0.5)", ambientColor: "140,220,255", ambientMode: 'rise', ambientShape: 'circle', bgStyle: 'city' },
    { type: 'glacier', name: "Gletser Kristal", plat: "#274a5e", top: "#cdeffb", lava: "#6fb8d9", sky1: "#9fd8ee", sky2: "#eefcff",
      mountain: "rgba(180, 230, 250, 0.4)", ambientColor: "220,250,255", ambientMode: 'fall', ambientShape: 'circle', bgStyle: 'iceberg' },
    { type: 'upsidedown', name: "Dimensi Terbalik", plat: "#1a0a2e", top: "#ff2fb0", lava: "#5c1a8f", sky1: "#0d0518", sky2: "#2a0a3d",
      mountain: "rgba(30, 5, 50, 0.6)", ambientColor: "255,60,190", ambientMode: 'glitch', ambientShape: 'square', bgStyle: 'glitchblocks' },
    { type: 'robotfactory', name: "Pabrik Robot", plat: "#3a3a3f", top: "#ffb020", lava: "#c0392b", sky1: "#232326", sky2: "#4a4a52",
      mountain: "rgba(40, 40, 45, 0.6)", ambientColor: "255,180,50", ambientMode: 'glitch', ambientShape: 'square', bgStyle: 'towers' },
    { type: 'bamboo', name: "Hutan Bambu Zen", plat: "#3a4a2a", top: "#a8d16a", lava: "#6b8a3a", sky1: "#dff0d8", sky2: "#f5faf0",
      mountain: "rgba(60, 80, 40, 0.35)", ambientColor: "200,230,170", ambientMode: 'fall', ambientShape: 'circle', bgStyle: 'trees' },
    { type: 'nirvana', name: "Nirwana Kosmik", plat: "#2a1a4a", top: "#ffe08a", lava: "#c9822a", sky1: "#1a0d33", sky2: "#4a2d6b",
      mountain: "rgba(40, 20, 60, 0.5)", ambientColor: "255,224,170", ambientMode: 'drift', ambientShape: 'circle', bgStyle: 'planets' },
    { type: 'futurecity', name: "Kota Kaca Masa Depan", plat: "#1a1f2e", top: "#5eead4", lava: "#0891b2", sky1: "#0c1222", sky2: "#1e293b",
      mountain: "rgba(20, 30, 50, 0.55)", ambientColor: "94,234,212", ambientMode: 'drift', ambientShape: 'square', bgStyle: 'city' },
    { type: 'fossilforest', name: "Hutan Fosil Purba", plat: "#3f3226", top: "#a68a5b", lava: "#7c5e3c", sky1: "#5c4a35", sky2: "#8a7355",
      mountain: "rgba(60, 45, 25, 0.5)", ambientColor: "210,190,150", ambientMode: 'fall', ambientShape: 'circle', bgStyle: 'trees' },
    { type: 'stormpalace', name: "Istana Awan Petir", plat: "#2b2d42", top: "#f4d35e", lava: "#ee964b", sky1: "#1a1a2e", sky2: "#3a3d5c",
      mountain: "rgba(30, 30, 55, 0.55)", ambientColor: "244,211,94", ambientMode: 'glitch', ambientShape: 'square', bgStyle: 'towers' },
    { type: 'iceplains', name: "Padang Es Abadi", plat: "#dbeeff", top: "#ffffff", lava: "#9fd8f0", sky1: "#bfe4ff", sky2: "#eef8ff",
      mountain: "rgba(190, 225, 250, 0.4)", ambientColor: "255,255,255", ambientMode: 'fall', ambientShape: 'circle', bgStyle: 'iceberg' },
    { type: 'robotruins', name: "Reruntuhan Robot Kuno", plat: "#33302a", top: "#c2a15a", lava: "#8a5a2a", sky1: "#26221c", sky2: "#4a4238",
      mountain: "rgba(50, 45, 30, 0.55)", ambientColor: "230,200,140", ambientMode: 'glitch', ambientShape: 'square', bgStyle: 'ruins' },
    { type: 'neonreef', name: "Terumbu Neon", plat: "#001f3f", top: "#ff2e88", lava: "#00e0ff", sky1: "#000c1f", sky2: "#031a3f",
      mountain: "rgba(0, 30, 60, 0.55)", ambientColor: "0,224,255", ambientMode: 'rise', ambientShape: 'circle', bgStyle: 'coral' },
    { type: 'crystaldesert', name: "Gurun Kristal", plat: "#6b4f7a", top: "#e0aaff", lava: "#9d4edd", sky1: "#ffd6a5", sky2: "#e0c3fc",
      mountain: "rgba(100, 60, 130, 0.4)", ambientColor: "224,170,255", ambientMode: 'drift', ambientShape: 'circle', bgStyle: 'crystal' },
    { type: 'floatingcity', name: "Kota Terapung", plat: "#3d5a80", top: "#98c1d9", lava: "#ee6c4d", sky1: "#e0fbfc", sky2: "#98c1d9",
      mountain: "rgba(60, 90, 130, 0.35)", ambientColor: "255,255,255", ambientMode: 'drift', ambientShape: 'circle', bgStyle: 'city' },
    { type: 'mushroomforest', name: "Hutan Jamur Raksasa", plat: "#3a1f47", top: "#ff6ec7", lava: "#c026d3", sky1: "#1f0a2e", sky2: "#4a1d5c",
      mountain: "rgba(50, 20, 60, 0.5)", ambientColor: "255,180,230", ambientMode: 'rise', ambientShape: 'circle', bgStyle: 'mushroom' },
    { type: 'shatteredmirror', name: "Dimensi Cermin Pecah", plat: "#1c1c24", top: "#c9c9ff", lava: "#7a7aff", sky1: "#0a0a12", sky2: "#22223a",
      mountain: "rgba(40, 40, 70, 0.5)", ambientColor: "200,200,255", ambientMode: 'glitch', ambientShape: 'square', bgStyle: 'crystal' }
];

let dimensionMode = localStorage.getItem('edward_dimension_mode') || 'random';
function activeDimensionMode() {
    return isMultiplayer ? (mpDimensionMode || 'random') : dimensionMode;
}
...
function renderDimensionPicker() {
    ...
    const isRandomSelected = dimensionMode === 'random';
    grid.innerHTML += `
        <div class="dim-card random-card ${isRandomSelected ? 'selected' : ''}" onclick="selectDimensionMode('random')">
            <div class="dim-preview"></div>
            <div class="dim-name">🎲 DIMENSI RANDOM</div>
            ${isRandomSelected ? `<div class="dim-check">✔ DIPAKAI</div>` : ''}
        </div>`;

    themes.forEach((theme, idx) => {
        const isSelected = !isRandomSelected && parseInt(dimensionMode, 10) === idx;
        grid.innerHTML += `
            <div class="dim-card ${isSelected ? 'selected' : ''}" onclick="selectDimensionMode('${idx}')">
                <div class="dim-preview" style="background: linear-gradient(180deg, ${theme.sky1}, ${theme.sky2});">
                    <div class="dim-ground" style="background: linear-gradient(180deg, ${theme.top}, ${theme.plat});"></div>
                </div>
                <div class="dim-name">${theme.name}</div>
                ${isSelected ? `<div class="dim-check">✔ DIPAKAI</div>` : ''}
            </div>`;
    });
}

window.selectDimensionMode = function(mode) {
    initAudio();
    dimensionMode = mode;
    localStorage.setItem('edward_dimension_mode', dimensionMode);
    playSfx(400, 600, 'square', 0.1, 0.05);
    renderDimensionPicker();
};

document.getElementById('openDimBtn').addEventListener('click', () => {
    initAudio(); document.getElementById('dimensionModal').style.display = 'flex';
    setTimeout(() => document.getElementById('dimensionModal').style.opacity = '1', 10); renderDimensionPicker();
});
document.getElementById('closeDimBtn').addEventListener('click', () => {
    initAudio(); document.getElementById('dimensionModal').style.opacity = '0';
    setTimeout(() => document.getElementById('dimensionModal').style.display = 'none', 300);
});
...
function resetGame() {
    ...
    currentThemeIndex = (activeDimensionMode() === 'random') ? 0 : (parseInt(activeDimensionMode(), 10) % themes.length);
    let dimBadge = document.getElementById('dimensionHUD');
    if (dimBadge) dimBadge.innerText = "🌀 " + themes[currentThemeIndex].name;

    particles = []; floatingTexts = [];
    keys = { left: false, right: false, jumpPressed: false };
    cameraX = 0; gameRunning = true; isPaused = false; currentDimension = 0; dimensionFlash = 0;
...
function drawPlayerSkin(ctx, p, legSwing, skinId, baseColor) {
    ...
    const drawArm = (c, isBack) => {
        ...
        if (p.isDashing) { endX = startX - 12; endY = -26; } 
            ...
            let liftY = (!p.grounded && p.vy < 0) ? -6 : 0;
...
function triggerGameOver() {
    ...
    document.getElementById('playAgainBtn').style.display = isMultiplayer ? 'none' : 'block';
...
function gameLoop() {
    ...
    if (p.dashCooldown === 0) {
        if (!gameLoop._dashBtn) gameLoop._dashBtn = document.getElementById('dashBtn');
        gameLoop._dashBtn.classList.remove('cooldown');
    }
    ...
    if (activeDimensionMode() === 'random') {
        let dimCheck = Math.floor(distance / 250);
        if(dimCheck > currentDimension && distance > 0) {
            currentDimension = dimCheck; dimensionFlash = 1;
            let nextIndex;
            if (isMultiplayer) {
                let pickRoll = mulberry32((mpSeed + dimCheck * 104729) >>> 0)();
                nextIndex = Math.floor(pickRoll * themes.length);
            } else {
                nextIndex = Math.floor(Math.random() * themes.length);
            }
            if (themes.length > 1 && nextIndex === currentThemeIndex) nextIndex = (nextIndex + 1) % themes.length;
            currentThemeIndex = nextIndex;
            let enteredTheme = themes[currentThemeIndex];
    ...
    let curTheme = (activeDimensionMode() === 'random') ? themes[currentThemeIndex % themes.length] : themes[parseInt(activeDimensionMode(), 10) % themes.length];
    ...
    let bgStyle = curTheme.bgStyle || 'mountain';
    bgElements.forEach(bg => {
        let drawX = bg.x - cameraX * 0.3;
        let baseY = viewportH - 60;
        let w = 150 * bg.scale;
        let h = 240 * bg.scale + (bg.seed % 50);
        ctx.fillStyle = curTheme.mountain || "rgba(0, 0, 0, 0.35)";

        if (bgStyle === 'city') {
            let bw = w * 0.4;
            let bh = h * 0.7;
            ctx.fillRect(drawX, baseY - bh, bw, bh);
            ctx.fillStyle = `rgba(${curTheme.ambientColor},0.55)`;
            for (let wy = baseY - bh + 10; wy < baseY - 8; wy += 16) {
                for (let wx = drawX + 6; wx < drawX + bw - 6; wx += 14) {
                    if ((Math.floor(wx + wy + bg.seed) % 3) === 0) ctx.fillRect(wx, wy, 6, 8);
                }
            }
        } else if (bgStyle === 'dunes') {
            ctx.beginPath();
            ctx.moveTo(drawX, baseY);
            ctx.quadraticCurveTo(drawX + w * 0.5, baseY - h * 0.85, drawX + w, baseY);
            ctx.fill();
        } else if (bgStyle === 'crystal') {
            ctx.beginPath();
            ctx.moveTo(drawX, baseY);
            ctx.lineTo(drawX + w * 0.35, baseY - h);
            ctx.lineTo(drawX + w * 0.5, baseY - h * 0.65);
            ctx.lineTo(drawX + w * 0.65, baseY - h);
            ctx.lineTo(drawX + w, baseY);
            ctx.fill();
        } else if (bgStyle === 'trees') {
            let trunkW = 10 * bg.scale;
            ctx.fillRect(drawX + w * 0.5 - trunkW / 2, baseY - h * 0.35, trunkW, h * 0.35);
            ctx.beginPath(); ctx.arc(drawX + w * 0.5, baseY - h * 0.55, w * 0.38, 0, Math.PI * 2); ctx.fill();
        } else if (bgStyle === 'coral') {
            for (let b = 0; b < 3; b++) {
                ctx.beginPath();
                ctx.ellipse(drawX + w * (0.2 + b * 0.3), baseY - h * (0.3 + b * 0.15), w * 0.16, h * (0.35 + b * 0.1), 0, 0, Math.PI * 2);
                ctx.fill();
            }
        } else if (bgStyle === 'ruins') {
            let pw = w * 0.2;
            ctx.fillRect(drawX, baseY - h * 0.8, pw, h * 0.8);
            ctx.fillRect(drawX + w - pw, baseY - h * 0.55, pw, h * 0.55);
        } else if (bgStyle === 'towers') {
            let tw = w * 0.16;
            ctx.fillRect(drawX + w * 0.4, baseY - h, tw, h);
            ctx.beginPath(); ctx.moveTo(drawX + w * 0.4, baseY - h); ctx.lineTo(drawX + w * 0.4 + tw / 2, baseY - h - 24 * bg.scale); ctx.lineTo(drawX + w * 0.4 + tw, baseY - h); ctx.fill();
        } else if (bgStyle === 'iceberg') {
            ctx.beginPath();
            ctx.moveTo(drawX, baseY);
            ctx.lineTo(drawX + w * 0.3, baseY - h);
            ctx.lineTo(drawX + w * 0.6, baseY - h * 0.6);
            ctx.lineTo(drawX + w, baseY);
            ctx.fill();
        } else if (bgStyle === 'planets') {
            ctx.beginPath(); ctx.arc(drawX + w * 0.5, baseY - h * 0.6, w * 0.28, 0, Math.PI * 2); ctx.fill();
        } else if (bgStyle === 'spikes') {
            ctx.beginPath();
            ctx.moveTo(drawX, baseY);
            ctx.lineTo(drawX + w * 0.5, baseY - h * 1.1);
            ctx.lineTo(drawX + w, baseY);
            ctx.fill();
        } else if (bgStyle === 'glitchblocks') {
            let jitter = (Math.floor(Date.now() / 150 + bg.seed) % 2 === 0) ? 6 : -6;
            ctx.fillRect(drawX + jitter, baseY - h * 0.7, w * 0.6, h * 0.7);
        } else if (bgStyle === 'mushroom') {
            let stemW = w * 0.12;
            ctx.fillRect(drawX + w * 0.5 - stemW / 2, baseY - h * 0.4, stemW, h * 0.4);
            ctx.beginPath(); ctx.ellipse(drawX + w * 0.5, baseY - h * 0.4, w * 0.32, h * 0.2, 0, 0, Math.PI * 2); ctx.fill();
        } else if (bgStyle === 'volcano') {
            ctx.beginPath(); ctx.moveTo(drawX, baseY); ctx.lineTo(drawX + w * 0.5, baseY - h); ctx.lineTo(drawX + w, baseY); ctx.fill();
            ctx.fillStyle = curTheme.lava; ctx.globalAlpha = 0.6;
            ctx.beginPath(); ctx.arc(drawX + w * 0.5, baseY - h * 0.95, 6 * bg.scale, 0, Math.PI * 2); ctx.fill();
            ctx.globalAlpha = 1;
        } else {
            ctx.beginPath(); ctx.moveTo(drawX, baseY); ctx.lineTo(drawX + w * 0.5, baseY - h); ctx.lineTo(drawX + w, baseY); ctx.fill();
        }
